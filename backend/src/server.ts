...
/**
 * ✅ CRITICAL FIX (Render / Proxy support)
 */
...
app.get("/api/health", (req, res) => {
  res.json({
    status: "ok",
    message: "Bharat 24/7 API is running 🚀",
  });
});

/**
 * 🖼 IMAGE PROXY (for external news images)
 */
...
wss.on("connection", (ws) => {
  ...
  console.log(
    `[WS] Viewer connected. Active: ${(global as any).activeViewers}`
  );
  ...
  ws.on("close", () => {
    ...
    console.log(
      `[WS] Viewer disconnected. Active: ${(global as any).activeViewers}`
    );
  });
...
server.listen(port, () => {
  ...
  console.log(
    `[WS] WebSocket viewer tracking active at ws://localhost:${port}/ws/viewers`
  );
});